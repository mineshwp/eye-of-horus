/* global React */
// Shared UI primitives + icons for Eye of Horus
...
const Icon = ({ name, size = 16, stroke = 1.6, style }) => {
  ...
  const paths = {
    ...
    sites: <><rect x="3" y="4" width="18" height="16" rx="2"/><path d="M3 9h18"/><circle cx="6.5" cy="6.5" r="0.5" fill="currentColor"/><circle cx="8.5" cy="6.5" r="0.5" fill="currentColor"/></>,
    issue: <><path d="M12 9v4"/><path d="M12 17h.01"/><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0Z"/></>,
    wp: <><circle cx="12" cy="12" r="9"/><path d="M3 12h18"/><path d="M12 3a14 14 0 0 1 0 18"/><path d="M12 3a14 14 0 0 0 0 18"/></>,
    diff: <><rect x="3" y="3" width="8" height="18" rx="1.5"/><rect x="13" y="3" width="8" height="18" rx="1.5"/><path d="M11 12h2"/></>,
    ...
    sparkles: <><path d="m12 3 1.9 4.6L18.5 9.5l-4.6 1.9L12 16l-1.9-4.6L5.5 9.5l4.6-1.9Z"/><path d="M19 14l.7 1.7L21.5 16.4l-1.7.7L19 19l-.7-1.7-1.7-.7 1.7-.7Z"/><path d="M5 16l.5 1.2L7 17.7l-1.2.5L5 19.4l-.5-1.2-1.2-.5L4.5 17.2Z"/></>,
...
// A small original glyph that ALSO appears alongside the photographic logo.
const HorusGlyph = ({ size = 28, color = "#D9A05B", glow = true }) => (
  <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
    <defs>
      <radialGradient id="hg-iris" cx="50%" cy="50%" r="50%">
        <stop offset="0%" stopColor="#FFE9B0"/>
        <stop offset="60%" stopColor={color}/>
        <stop offset="100%" stopColor="#A06900"/>
      </radialGradient>
      {glow && <filter id="hg-glow" x="-50%" y="-50%" width="200%" height="200%">
        <feGaussianBlur stdDeviation="1.2" result="b"/>
        <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
      </filter>}
    </defs>
    <g filter={glow ? "url(#hg-glow)" : undefined}>
      {/* Top eyelid */}
      ...
      {/* Iris */}
      <circle cx="16" cy="16" r="4" fill="url(#hg-iris)"/>
...
// ============ Sidebar ============
const NavItem = ({ icon, label, count, critical, active, onClick }) => (
...
const Sidebar = ({ route, setRoute }) => {
  ...
  const criticalCount = D.issues.filter(i => i.severity === "critical").length;
  const wpCount = D.wpUpdates.length;
  return (
    <aside className="sidebar">
      <div className="sidebar-brand">
        <div className="mark"><img src="assets/horus-mark.png" alt=""/></div>
        <div className="wordmark">Eye of Horus<span className="sub">Command Centre</span></div>
      </div>

      <div className="sidebar-section">
        <div className="sidebar-section-label">Monitor</div>
        <NavItem icon="dashboard" label="Dashboard"   active={route.name === "dashboard"} onClick={() => setRoute({ name: "dashboard" })} />
        <NavItem icon="sites"     label="Websites"    count={D.sites.length} active={route.name === "site" || route.name === "sites"} onClick={() => setRoute({ name: "site", id: "acme" })} />
        <NavItem icon="issue"     label="Issues"      count={criticalCount} critical active={route.name === "issue"} onClick={() => setRoute({ name: "issue", id: "i1" })} />
        <NavItem icon="diff"      label="Visual changes" active={route.name === "regression"} onClick={() => setRoute({ name: "regression" })} />
        <NavItem icon="wp"        label="WP updates"  count={wpCount} active={route.name === "wp"} onClick={() => setRoute({ name: "wp" })} />
      </div>

      <div className="sidebar-section">
        <div className="sidebar-section-label">Workspace</div>
        <NavItem icon="reports"  label="Reports"     active={route.name === "reports"} onClick={() => setRoute({ name: "reports" })}/>
        <NavItem icon="settings" label="Monitoring"  active={route.name === "settings"} onClick={() => setRoute({ name: "settings" })}/>
      </div>
      ...
      <div className="sidebar-foot">
        <div className="user-card">
          ...
          <div style={{ flex: 1 }}>
            <div className="user-name">Mia Patel</div>
            <div className="user-role">QA Lead · Wetpaint</div>
          </div>
...
// ============ Topbar ============
const Topbar = ({ crumbs = [], onJumpSignIn, theme, onToggleTheme }) => (
  <header className="topbar">
    ...
    <button className="icon-btn" title="Run scan now"><Icon name="refresh" size={15}/></button>
    ...
    <button className="btn ghost sm" onClick={onJumpSignIn} title="Lock"><Icon name="user" size={13}/> Lock</button>
  </header>
);

// ============ Reusable bits ============
const Badge = ({ tone = "low", children, dot, lg }) => (
...
const ScoreBar = ({ value, max = 100 }) => {
  ...
  const color = value >= 90 ? "var(--green)" : value >= 75 ? "var(--cyan)" : value >= 60 ? "var(--amber)" : "var(--red)";
  return (
    <div className="score-bar">
      <div className="track"><div className="fill" style={{ width: `${pct}%`, background: color, boxShadow: `0 0 10px ${color}` }}/></div>
      <span style={{ color }}>{value}</span>
    </div>
  );
};

const Sparkline = ({ points, color = "#00E5FF", height = 36, fill = true }) => {
...
const KPI = ({ icon, label, value, unit, delta, deltaDir = "flat", glow = "rgba(0,229,255,0.18)", spark, sparkColor = "#00E5FF" }) => (
  <div className="card kpi-card">
    ...
    {spark && <div style={{ marginTop: 4 }}><Sparkline points={spark} color={sparkColor} height={30}/></div>}
  </div>
);
...
Object.assign(window, {
  Icon, HorusGlyph,
  Sidebar, Topbar,
  Badge, SeverityChip, StatusChip,
  ScoreBar, Sparkline, KPI, Toggle, Tabs, Favicon,
});
