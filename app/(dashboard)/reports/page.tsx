...
export default function ReportsPage() {
  ...
  const handleExportPDF = () => {
    alert("Compiling portfolio PDF report. Download will start shortly...");
    window.print();
  };

  const handleGenerateReport = async () => {
    if (!sites.length) { alert("No sites available to generate report for."); return; }
    setGenerating(true);
    setGenerateError(null);
    const site = sites[0]; // Use first site for demo
    try {
      ...
      <div className="page-head">
        <div>
          ...
          <p className="page-sub">
            Generate ready-to-send summaries for clients and the internal team. Trend cards show how the portfolio is moving over time.
          </p>
        </div>
  ...
  <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
    ...
    {reports.length === 0 ? (
      <div className="card card-pad" style={{ textAlign: "center", padding: 48 }}>
        <div className="muted">No reports generated yet. Click &ldquo;Generate report&rdquo; above to create your first report.</div>
      </div>
    ) : (
      <div className="card">
  ...
  <>
    ...
    <div className="grid-2" style={{ marginBottom: 18 }}>
      <div className="ai-callout">
        <span className="ai-tag">
          <Icon name="sparkles" size={11} /> Week summary · Horus
        </span>
        ...
        <ul style={{ margin: 0, paddingLeft: 18, color: "var(--text-secondary)", fontSize: 13.5, lineHeight: 1.75 }}>
          ...
          <li>
            16 issues resolved · median time-to-resolve <strong style={{ color: "var(--text-primary)" }}>4h 12m</strong>
          </li>
      ...
      <div className="card">
        <div className="card-head">
          ...
          <span className="h-sub">last 30 days</span>
        </div>
    ...
    <div className="grid-3">
      <PortfolioCard
        ...
        status="critical"
        health={64}
        bullets={["Mobile hero CTA missing — open", "SSL renews in 9 days — action needed", "2 plugin updates pending staging test"]}
      />
      <PortfolioCard
        site="Tarsus Cloud Portal"
        status="critical"
        health={58}
        bullets={["Form-Pro rolled back to 4.1.9 · forms restored", "JS error spike on /checkout — investigating", "WP core 2 versions behind"]}
      />
      <PortfolioCard
        site="Wetpaint Corporate"
        status="healthy"
        health={96}
        bullets={["All 18 pages clean · 7 days streak", "No pending updates", "Form success rate 100%"]}
      />
    </div>
  </>
);
...
const ClientReady = () => (
  <div className="grid-2">
    <div className="card" style={{ overflow: "hidden" }}>
      <div className="card-head">
        <h3>
          <Icon name="file" size={14} /> Acme Finance · Client-facing summary
        </h3>
      ...
      >
        ...
        <div style={{ color: "var(--text-tertiary)", fontSize: 13, marginBottom: 18 }}>Prepared by Wetpaint · powered by Eye of Horus</div>

        <ReportSection title="Highlights" items={["Uptime maintained at 99.96% across the week", "16 routine checks completed without action needed", "1 critical issue detected and fix in progress"]} />

        <ReportSection
          title="What we're actioning"
          items={["Mobile hero button has been temporarily hidden by a theme update — restoring this week", "SSL certificate renewal scheduled before expiry"]}
        />

        <ReportSection title="Coming up" items={["WooCommerce 9.0 upgrade scheduled on staging", "Quarterly performance review"]} />
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <div className="card">
        ...
        <div className="card-pad">
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
            <button className="btn full" onClick={() => alert("Preview email sent to qa-leads@wetpaint.co.za")} type="button">
              Preview email
            </button>
            <button className="btn primary full" onClick={() => alert("Report successfully dispatched to client stakeholders.")} type="button">
              Send to client
            </button>
          </div>
        </div>
      ...
      <div className="ai-callout">
        ...
        <div style={{ marginTop: 8, fontSize: 13, color: "var(--text-secondary)", lineHeight: 1.6 }}>
          The Acme team prefers Friday reports. Schedule this to send{" "}
          <strong style={{ color: "var(--text-primary)" }}>every Friday at 16:00 SAST</strong> with the same options?
        </div>
        <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
          <button className="btn primary sm" onClick={() => alert("Scheduled weekly report for Acme Finance.")} type="button">
            Schedule weekly
          </button>
...
const InternalDev = () => (
  <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
    <div className="card">
      <div className="card-head">
        <h3>
          <Icon name="code" size={14} /> Engineering digest · Week of 12 May
        </h3>
        <Badge tone="info">For #wetpaint-eng</Badge>
      </div>
      <div className="card-pad" style={{ fontSize: 13.5, lineHeight: 1.7, color: "var(--text-secondary)" }}>
        <ReportSection
          title="Outages & rollbacks"
          items={["Tarsus Cloud Portal — Form-Pro 4.2.1 rolled back to 4.1.9 (form failures). Owner: J. Ndlovu.", "No production outages this week."]}
        />
        <ReportSection
        ...
        />
        <ReportSection title="Stack drift" items={["5 sites running ACF < 6.3.4", "3 sites using PHP 8.2 — supported but Gentech can move to 8.3 next quarter"]} />
      </div>
    </div>
...
const Trends = () => (
  <div className="grid-2eq">
    <div className="card">
      <div className="card-head">
        <h3>Performance trend · portfolio avg</h3>
        <span className="h-sub">90 days</span>
      </div>
      <div className="card-pad">
        <Sparkline points={[78, 79, 80, 81, 80, 79, 78, 78, 79, 80, 79, 78, 77, 78, 79, 80, 81, 80, 79, 78, 77, 76, 75, 76, 77, 78, 79, 78, 77, 76]} color="#F59E0B" height={140} />
      </div>
    </div>
    <div className="card">
      <div className="card-head">
        <h3>Security score · portfolio avg</h3>
        <span className="h-sub">90 days</span>
      </div>
      <div className="card-pad">
        <Sparkline points={[82, 82, 83, 83, 84, 84, 85, 84, 85, 86, 86, 87, 87, 87, 86, 86, 87, 88, 88, 89, 89, 89, 90, 90, 90, 91, 90, 90, 91, 91]} color="#22C55E" height={140} />
      </div>
    </div>
    <div className="card">
      <div className="card-head">
        <h3>Form reliability</h3>
        <span className="h-sub">14 days</span>
      </div>
    ...
    <div className="card">
      <div className="card-head">
        <h3>Issues by category · last 30 days</h3>
      </div>
      <div className="card-pad" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
